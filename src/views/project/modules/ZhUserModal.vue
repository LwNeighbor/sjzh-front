    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="账号">
          <a-input placeholder="请输入账号" v-decorator="['username', validatorRules.username ]" />
        </a-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        add: '/zhuser/zhUser/add',
        edit: '/zhuser/zhUser/edit'
      }
    }
  },
  created() {},
  methods: {
    ...
    edit(record) {
      ...
      this.visible = true
      this.$nextTick(() => {
        this.form.setFieldsValue(pick(this.model, 'password', 'username'))
        //时间格式化
      })
    },
    close() {
